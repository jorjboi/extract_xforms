...
print("POOPOO")
print("extract_anim loaded from:", extract_anim.__file__)
...
def create_outputs(parent_node, input_node):
    ...
    print(attr_name)
    print(input_node.geometry())
#    print([a.name() for a in input_node.geometry().globalAttribs()])
 #   print([a.name() for a in input_node.geometry().pointAttribs()])
    #total_parts = input_node.geometry().attribValue(attr_name)
    values = input_node.geometry().pointIntAttribValues(attr_name)  # or .pointFloatAttribValues() or .pointStringAttribValues()
    total_parts = len(set(values))
...
def extract_anims(parent_node, moving_parts):
...
def extract(alembic_path):
    ...
    extract_anims(subnet_node, parts)
